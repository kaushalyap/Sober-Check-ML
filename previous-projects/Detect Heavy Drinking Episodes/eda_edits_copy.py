...
FeatureType = {Features.Mean: 'Mean', Features.Median: 'Median', Features.Std_Dev: 'Std_Dev',
               Features.ZeroCrsRate: 'ZeroCrsRate', Features.Max_Raw: 'Max_Raw', Features.Min_Raw: 'Min_Raw',
               Features.Max_Abs: 'Max_Abs', Features.Min_Abs: 'Min_Abs', Features.Spec_Ent_Time: 'Spec_Ent_Time',
               Features.Spec_Ent_Freq: 'Spec_Ent_Freq', Features.Spec_Cent: 'Spec_Cent',
               Features.Spec_Sprd: 'Spec_Sprd', Features.Spec_Flux: 'Spec_Flux', Features.Spec_RollOff: 'Spec_RollOff',
               Features.Max_freq: 'Max_freq', Features.Skewness: 'Skewness', Features.Kurtosis: 'Kurtosis',
               Features.Avg_Power: 'Avg_Power'}


def separate_pid_data():
    acc_data = pd.read_csv("datasets/accelerometer/all_accelerometer_data_pids_13.csv")
...
def create_per_second_data(pid_filename, metric_no):
    ...
    for idx in range(0, tot_rows):
        ...
        if idx != 0:
            prev_ts = acc_data.loc[idx - 1, 'time'] % 1000

        if curr_ts > prev_ts:
            sub_frame.append([r['time'], r['x'], r['y'], r['z']])

        else:
            # Do calculations for all entries of one second
            ...
            # Add the last timestamp in that second window
...
def create_per_window_data(file_name, metric_no):
    ...
    single_row = list()
    i = 0
    print("Shape of data obtained from pickle - ", mean_all.shape)
    ...
    col_names = ['xMe', 'xVr', 'xMx', 'xMi', 'xUM', 'xLM', 'yMe', 'yVr', 'yMx', 'yMn', 'yUM', 'yLM', 'zMe', 'zVr',
                 'zMx', 'zMi', 'zUM', 'zLM']
    ...
    if metric_no <= 13:
        ...
        for i in range(len(full_frame)):
            if i == 0:
                diff_frame.append(full_frame[:1, 1:][0])
            else:
        ...
        output_file_name = "Pickles/Metric_" + str(metric_no) + "_36.pkl"
    else:
        result_df = df1
        output_file_name = "Pickles/Metric_" + str(metric_no) + "_18.pkl"

    print(result_df.shape)


# if __name__ == "__main__":
#     # pids = separate_pid_data()
#     pidFile = 'data/BK7610'
#     for i in Features:
#         if i.value == 12:
#             continue
#         create_per_second_data(pidFile, i.value)
#         file_per_window = create_per_window_data(file_per_sec, i.value)
